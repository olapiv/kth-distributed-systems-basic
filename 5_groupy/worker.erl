...
start(Id, Module, Rnd, Peer, Sleep) ->
    spawn(fun() -> init(Id, Module, Rnd, Peer, Sleep) end).  % Spawns master-process

init(Id, Module, Rnd, Peer, Sleep) ->
    % In master process now.

    {ok, Cast} = apply(Module, start, [Id, Peer]),  % Returns {ok, slave pid}
...
worker(Id, Cast, Color, Gui, Sleep) ->
	...
	%% Someone wants to join our group - the leader needs to know.
    %% This is called if the masters pid was used when adding a worker.
	{join, Peer, Gms} ->
	    Cast ! {join, Peer, Gms},
	    worker(Id, Cast, Color, Gui, Sleep);	    
